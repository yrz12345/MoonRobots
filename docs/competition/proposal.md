...
MoonRobots 将这些规则封装为可复用 MoonBit API，并保留命中规则、源码行号和决策原因。项目还可以在上线前比较新旧策略，找出意外开放或关闭的 URL，并使用真实 URL 语料识别未覆盖和被遮蔽的规则。它既可作为其他 MoonBit 网络项目的依赖，也可作为独立 CLI 帮助开发者和网站管理员建立可重复的策略验收流程。
...
6. 提供 check、inspect、lint、batch、coverage、diff 命令及稳定 JSON 输出。
...
10. 对新旧策略执行 URL 语料回归，报告新增允许和新增拒绝。
11. 统计规则匹配与胜出次数，识别从未匹配和动态遮蔽规则。

## 四、技术方案

核心库由五层组成：容错解析器、路径规范化器、线性通配符匹配器、可解释决策引擎和策略分析器。策略分析器复用单 URL 判定结果，聚合为回归变化和动态覆盖率，不在 CLI 中复制规则语义。CLI 和浏览器实验台仅负责输入及结果展示，确保同一套核心逻辑可被浏览器、服务端、CI 和 Agent 工具复用。浏览器端通过稳定的字符串 JSON 边界调用 MoonBit 生成的 IIFE 模块，无需后端服务。
...
后续版本将增加 HTTP 获取与缓存、Sitemap 解析、历史趋势报告，以及供 AI Agent 调用的 MCP Server。项目不会把 `robots.txt` 描述为安全或授权机制。
